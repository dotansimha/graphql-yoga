---
id: 04-a-simple-query
title: A simple Query
sidebar_label: A simple Query
type: Tutorial
---

In this section, you are going to implement the first GraphQL object types and fields that provides the functionality of a Hacker News
clone for querying a feed of _links_ that were posted by other users.

### Extending the schema definition

Let's start by implementing a `feed` field on the `Query` root types which allow retrieving a list of `Link` elements. In general, when adding a new feature to the GraphQL Schema, the process will look pretty similar every time:
...
1. Implement corresponding _resolver functions_ for the added fields/object types
...
The next step is to implement the resolver function for the `feed` query field. In fact, one thing we haven't mentioned yet is
...
Go ahead and test the implementation by running the server and navigate to `http://localhost:4000/graphql` in your browser. If you expand the documentation of the GraphiQL, you'll notice that another field on the root `Query` object type called `feed` is now available:

[![another `Query` root type field called feed](/assets/tutorial/basic/05-a-simple-mutation/feed-link-query-response.png)](/assets/tutorial/basic/05-a-simple-mutation/feed-link-query-response.png)
...
[![data returned from the server](/assets/tutorial/basic/04-a-simple-query/feed-query-response.png)](/assets/tutorial/basic/04-a-simple-query/feed-query-response.png)
...
Let's now quickly talk about how a GraphQL server actually resolves incoming requests. As you already saw, a GraphQL operation contains a selections set of _fields_ that have their source in the type definitions of the GraphQL schema.
...
Can you imagine what the query resolution process looks like now? Effectively, the GraphQL server has to invoke all resolver functions for the fields that are selection in the operation and then package up the response according to the selection sets shape. The resolution thus merely becomes a process of orchestrating the invocation of resolver
functions!

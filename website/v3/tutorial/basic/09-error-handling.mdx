...
For doing so you will use the `GraphQLYogaError` class that is exported from the `graphql-yoga` package.
...
That means every time you want to expose an error to the outside world you should throw an `GraphQLYogaError` error.

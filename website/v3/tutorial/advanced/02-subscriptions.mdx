...
You'll be using a simple `PubSub` implementation from the `graphql-yoga` library to implement subscriptions to the following _events_:
...
The last thing you need to do for our subscription implementation itself is to actually trigger that `newLink` event from within our code!
...
Similar to mutation and query operations over HTTP, subscriptions also result in a HTTP response. However, compared to thsoe other two oepration types the HTTP response is a long-lived connection that must be terminated by the user.

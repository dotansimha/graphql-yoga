...
> If you want to avoid exeucuting arbitary GraphQL operations please use the Persisted Documents plugin.

## Installation

<PackageInstall
  packages={['@graphql-yoga/plugin-automatic-persisted-queries']}
/>
...
import { useAutomaticPersistedQueries } from '@graphql-yoga/plugin-automatic-persisted-queries'

const yoga = createYoga({
  plugins: [useAutomaticPersistedQueries()],
})
...
const store: AutomaticPersistedQueriesStore = new Map()

const yoga = createYoga({
  plugins: [useAutomaticPersistedQueries({ store })],
})

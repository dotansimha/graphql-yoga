...
GraphQL Yoga understands [WHATWG `Request` object](https://developer.mozilla.org/en-US/docs/Web/API/Request) object to consume your HTTP request. Let's say your environment exposes something like below;
...
export function myMiddleware(req: MyRandomRequest, ctx: MyRandomCtx) {
  ...
  const request = new Request(req.url, {
    method: req.method,
    headers: req.headers,
    body: req.body, // req.body should be a valid BodyInit like a string or a Buffer etc...
  })
  ...
  // Let's say your environment needs to return something like below;
  return {
    ...
    // body: response.body if it accepts a ReadableStream

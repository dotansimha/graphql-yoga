...
<PackageInstall packages={['graphql-yoga@alpha']} />

### Install equivalent **Envelop** plugins of the Apollo Server

- `disableIntrospection: true` flag
  - If you are using this flag, [install `@envelop/disable-introspection`](https://www.envelop.dev/plugins/use-disable-introspection)
- Apollo Federation
  - If you are using Apollo Server errors, [install `@envelop/use-apollo-federation`](https://www.envelop.dev/plugins/use-apollo-federation)
...
[Check more plugins on Envelop Plugin Hub](https://www.envelop.dev/plugins)
...
+ const server = new GraphQLYoga({
  ...
  schema,
+  // Add here the plugins you installed above

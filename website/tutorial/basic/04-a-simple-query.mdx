...
![another `Query` root type field called feed](https://i.imgur.com/k60k4BC.png)
...
Let's now quickly talk about how a GraphQL server actually resolves incoming requests. As you already saw, a GraphQL
operation contains a selections set of _fields_ that have their source in the type definitions of the GraphQL schema.
...
All four fields specified in the query operations selection set (`feed`, `id`, `url`, and `description`) can also be found inside the schema
definition. You also learned that _every_ field inside the schema definition is backed by one resolver function
...
invoke all resolver functions for the fields that are selection in the operation and then package up the response according
to the selection sets shape. The resolution thus merely becomes a process of orchestrating the invocation of resolver
functions!
...
On the first level, it invokes the `Query.feed` resolver and returns the entire data stored in `links`. For the second

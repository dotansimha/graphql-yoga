---
id: 10-filtering-and-pagination
title: Filtering and Pagination
sidebar_label: Filtering and Pagination
---
...
Go ahead and add the `filter` argument definition of the type `String` to the `Query.feed` field definition within in your application schema (under `src/schema.ts`):
...
[docs](https://www.prisma.io/docs/reference/tools-and-interfaces/prisma-client/pagination)). In this tutorial, you're going to implement limit-offset pagination.
...
As more links are posted onto the feed, the size of the feed increases. At some point there could be thousands of feed items. When deploying the the GraphQL API to production you would not want to allow querying ALL the links at once. That means you have to introduce a default value for the `take` argument.
...
Cool! Now we you can try executing a operation with a `take` argument value outside the range!

---
id: 05-a-simple-mutation
title: A simple Mutation
sidebar_label: A simple Mutation
type: Tutorial
---
...
1. The implementation of the `postLink` resolver on the `Mutation` object type first creates a new `link` object, then adds it to the existing `links`
...
Correct! 💡 It carries the _arguments_ for the field that are sent – in this case the `url` and `description` of the `Link` to be
created. We didn't need it for the `feed` and `info` resolvers before, because these `Query` object type fields don't

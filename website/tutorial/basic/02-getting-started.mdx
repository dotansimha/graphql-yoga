...
- `typeDefinitions` - this is your GraphQL schema definition. You've created a `Query` type that exposes a field called `info`, of type `String`. You can import it directly from `.graphql` file thanks to `graphql-import-node`.
...
To use your GraphQL schema and consume data from it, you will need to write an executable GraphQL `query` definition.

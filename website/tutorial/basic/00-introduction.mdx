...
- `graphql-yoga`- the easiest way of running a GraphQL server.
...
corresponding _resolver functions_ for retriving data. In the beginning, these resolvers will only work with data that's stored in-memory -
...
Last but not least, we will make a part of the API real-time by adding GraphQL subscriptions!

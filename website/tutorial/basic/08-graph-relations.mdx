...
his command has now generated your second migration inside of `prisma/migrations`, and you can start to see how this becomes a historical record of how your database evolves over
...
type Mutation {
  ...
  postCommentOnLink(linkId: ID!): Comment!
}
```
...
1. **`linkId`**. This is the id of the link the comment belongs to. GraphQL has a special scalar for describing ids that you used before, called `ID`. In general it is a best practise to use it. The downside is that a GraphQL `ID` is always a string. Thus, within the resolver function you must parse the actual integer value of the `ID`, as the SQLite database uses integer values for IDs.
2. **body``**. This is the comment body, there is nothing special about it.

You probably can't wait to actually try sending a mutation for creating your first comment.
...
Executing the folloing mutation operation.
...
Instead you will first create resolver for retreiving a comment via it's id.
...
Add the corresponding logic for retriving a comment by its id.
...
So now you implemented fetching a comment by it's id.
...
Being able to fetch multiple resources within a single request is on of the main advantages of using GraphQL!

### Optional Exercise

As an additonal exercise you can gain more hands on experience by implementing the resolvers for `Comment.link` and `Query.link`.
...
If you are struggeling check out the implementations of the previous resolvers! There is nothing new happening here. You can do it!

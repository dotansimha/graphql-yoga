import { Callout, Instruction } from '@theguild/components'
...
Now, you're adding the global instance of your `PubSub` and making sure it's available for you during your GraphQL execution, by injecting it to your `context`, just as you stored an instance of `PrismaClient` in the variable `prisma`.
...
> **Note**: If you see a loading spinner indefinitely, it may be because of SSE disconnecting when the tab goes idle. Ensure that both the tabs are active.
  ...
  Mutation: {
    vote: async (
      ...
      if (!context.currentUser) {
        throw new GraphQLYogaError('You must login to use upvote!')
      }
...
1. Make sure that our server doesn't allow upvotes without being authenticated.

...
On the Prisma schema, we chose to make comments mandatory because of their best practices. So in
case there are no comments, it will return an empty string.

On the GraphQL schema, we can choose to follow the same practice, or to make the comments array
nullable.

In our example, we've choosen the nullable option, as it single to the consumer of our API that they
need to handle that case, when they will generate the types on their end.

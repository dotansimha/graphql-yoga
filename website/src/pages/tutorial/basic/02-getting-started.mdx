...
2. GraphQL types are connected using fields, and they form a graph.
...
<PackageCmd packages={['--save graphql @graphql-tools/schema']} />
...
A GraphQL schema can be written with GraphQL SDL (*S*chema *D*efinition *L*anguage), which is the GraphQL language for defining your API/contract. The actual code and business logic of each field are called a GraphQL _resolver_.

So let's get started by creating your first, very-simple, GraphQL schema.

To get started with a simple GraphQL schema, you need to create an SDL file defining our contract:
...
You can ignore the `[Object: null prototype]`. It just means that the object does not inherit any properties from the global Object prototype and is a security measure that is insignificant for this tutorial.
...
<Callout type="info">
  **Note**: In this tutorial, we'll only scratch the surface of this topic. If
  you want to go a bit more in-depth and learn more about the GraphQL schema as
  well as its role in a GraphQL API, be sure to check out

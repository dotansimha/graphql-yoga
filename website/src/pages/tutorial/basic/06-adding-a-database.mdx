...
You're going to set up a [SQLite](https://www.sqlite.org/index.html) to persist the data of incoming GraphQL mutations. Instead of writing SQL directly, you will
...
Prisma concepts will be kept light but we have included links to [Prisma docs](https://www.prisma.io/docs/) in case you want to dive deeper into any particular concept.
...
<PackageCmd packages={['--save-dev --save-exact prisma @prisma/client']} />
...
Open `schema.prisma` and replace the existing contents with the following code:
...
specific migration. This is then put inside that `prisma/migrations` directory, which becomes a historical reference of how your database evolves with each migration you make!
...
For each additional run, you will see one more item within the list. That is great, as it means our data is persisted outside of the Node.js memory!

...
- Have NodeJS installed on your operation system
  ...
  (14/16/17), but, we highly recommend using the stable Node.js 16 LTS release.
...
<PackageCmd packages={['-D --save-exact typescript @types/node ts-node ts-node-dev cross-env']} />
...
- `ts-node` and `ts-node-dev` are libraries that allow you to run `.ts` files directly, without a
  compilation step to JavaScript and automatically re-run your files after you apply file changes.

...
the directory that'll hold the files for your project:
...
<PackageCmd
  packages={[
    '--save-dev --save-exact typescript @types/node ts-node ts-node-dev cross-env',
  ]}
/>
...
- `@types/node` is a package that contains the basic TypeScript types for the NodeJS environment.

...
For doing so you will use the `GraphQLYogaError` class that is exported from the
`@graphql-yoga/node` package.
...
You use that code for identifying this edge case of a missing link and then throw a
`GraphQLYogaError` with a useful error message instead.
...
const resolvers = {
  ...
  Mutation: {
    ...
    async postCommentOnLink(
      ...
      if (linkId === null) {
        return Promise.reject(
          new GraphQLYogaError(`Cannot post comment on non-existing link with id '${args.linkId}'.`)
        )
      }

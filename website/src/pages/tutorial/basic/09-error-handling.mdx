import { Instruction } from '@theguild/components'
...
This is the resolver implementation that caused the issue when using a `linkId` that has no counterpart within the database.
...
In a production environment, a stack trace that leaks to the outside world are a potential security threat as that information could be misused by malicious actors.
...
You will receive a response with the `Cannot post a comment on the non-existing link with id '99999999999'.` message.
...
```json
{
  "errors": [
    {
      "message": "Cannot post a comment on the non-existing link with id 'uuuuuu'.",
      "locations": [
        {
          "line": 2,
          "column": 3

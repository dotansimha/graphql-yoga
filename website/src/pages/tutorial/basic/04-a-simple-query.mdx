...
All four fields specified in the query operations selection set (`feed`, `id`, `url`, and
...
package up the response according to the selection sets shape. The resolution thus merely becomes a
...
(i.e. nested curly braces) corresponds to one resolver execution level. The above query operation.
Therefore, has two of these execution levels.

...
3. `schema.generated.graphqls` contains the merged schema of your application. In this example,
   ...
   up into multiple smaller modules, each with its own schema file e.g.
   `src/schema/comment/schema.graphql`, `src/schema/link/schema.graphql`, etc.
...
As long as you keep the generated files up-to-date with your schema and mappers, you can be sure
typechecks work as expected.

...
If you setup a proxy or redirect to forward requests from `example.com/api/*` to `api.example.com`, you avoid CORS issues all together.
...
You can disable CORS on your Yoga server by simply passing `false` as the cors property. For example::

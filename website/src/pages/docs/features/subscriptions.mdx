...
import { type GraphQLResponse, Network, Observable, type RequestParameters } from 'relay-runtime'
...
you simply use the `@graphql-yoga/plugin-graphql-sse` plugin for GraphQL Yoga that exposes an
additional endpoint (defaulting to `/graphql/stream`) used for
...
<Callout>Check out [our working example](/examples/graphql-sse) for this integration.</Callout>
...
<Callout>Check out [our working example](/examples/graphql-ws) for this integration.</Callout>
...
import { createPubSub, createSchema, createServer } from 'graphql-yoga'
...
import { createPubSub, createSchema, createYoga, Repeater } from 'graphql-yoga'

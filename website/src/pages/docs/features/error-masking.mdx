...
Sometimes it is feasible to throw errors within your GraphQL resolvers whose message should be send
...
will never be included the the response extensions sent to the user.
...
You can add additional HTTP headers by specifying the `extensions.http.status` field. All the
...
a multipart operation (defer/stream) and only possible when when subscribing to the event source in
a subscription operation (the HTTP status of an ongoing request that already sent a status code can
not be changed). Please use the `extensions.http` with caution.
...
Executing the following operation will now result within a leaking error message that exposes

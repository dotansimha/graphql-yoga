...
<Callout>
  ...
  the GraphQL execution for example a plugin such as the [response-cache](TODO:
  response cache plugin link), can never act on the HTTP level. However, the
  yoga response-cache plugin utilizes the HTTP level hooks, and thus can
  completly skip all of the expensive GraphQL execution steps for serving cached
  results.
</Callout>
...
The most hooks for Yoga origin from the envelop plugin system. [Please refer to the Envelop Plugin documentation for more information.](TODO: add link)

...
stateDiagram-v2
    ...
    state GraphQLRequest {
      ...
      onParams --> onParse
      onParse --> onValidate
      onValidate --> onContext
      onContext --> onExecute
      onContext --> onSubscribe
...
This hook is invoked for any incoming GraphQL HTTP request and is invoked Invoked before attempting
to parse the GraphQL parameters. Here you can manipulate the request, set a custom request parser or
apply security measures such as checking for access tokens etc.

...
<Callout type="info">
  ...
  documentation](https://the-guild.dev/graphql/yoga-server/docs/features/persisted-operations).
...
## Validation of persisted operations

By default, the persisted operations plugin skips the validation for persisted operations as it is assumed that the persisted operations are already validated. However, you can still enable validation for persisted operations by setting the `skipValidation` option to `false`.

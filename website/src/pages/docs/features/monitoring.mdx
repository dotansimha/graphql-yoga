...
Each metric also expose a set of labels. All labels are exposed by default (to the exception of http
request and response headers) but can be separately disabled by setting the corresponding key in
`labels` option object to `false`.

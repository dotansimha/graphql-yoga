...
| `subscriptionsProtocol`    | `both` or `legacy` or `current`                                           | `'both'`                                                                                      | Which websocket subscriptions server and protocol to use. `legacy` for the legacy [`subscriptions-transport-ws`](https://github.com/apollographql/subscriptions-transport-ws) and `current` for [`graphql-ws`](https://github.com/enisdenjo/graphql-ws). Defaults to `both` which implements both protocols and decides depending on the client. |
